    ...
    _.each(cFights, (fights) => {
        ...
        _.each(fights, (fight) => {
            ...
            let matches = _.filter(aggregated, (aFight, aID) => {
                if (_.toLower(fight.target) == _.toLower(aFight.target) && fight.raidLeader == aFight.raidLeader) {
            ...
            if (matches.length > 0) {
                _.each(matches, (match, i) => {
                    aggregated[ids[i]] =  mergeFights(fight, match)
                })
            } else {
...
function mergeFights(a, b) {
    ...
    merged.raidLeader = a.raidLeader
    return merged
}
...
function calculateMergedAllies(participants) {
    ...
    while (!ran) {
        _.each(keys, (key) => {
            if (!_.get(participants, [key, "ally"]) && participants[key]) {
...
class EQParser {
    ...
    parse(data, client) {
        ...
        // data calculation is done later, on demand, instead an event is logged in the fight
